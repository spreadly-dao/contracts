...
class CompiledContract {
  constructor(compiledOutputString) {
    ...
    const compileOutputArray = compiledOutputString
      .split('\r\n')
      .filter((str) => str != '')
...
function compileContract(contract, symbols = '') {
  console.log(`Building static contract ${contract} ${symbols} ...`)
  var command = `java -cp ErgoScriptCompiler.jar Compile ${contract} ${symbols}`
  const compileOutput = execSync(command).toString()
  const compiledContract = new CompiledContract(compileOutput)
  console.log(`\x1b[32mSuccessfuly static compiled ${contract} ...\x1b[0m`);
...
for (var token of TOK.TOKENS) {
  ...
  if (token.oracleType === 'Oracle') {
    token.validOracle = fs
      .readFileSync('./templates/ValidOracle.mu.es')
      .toString()
  } else {
    token.validOracle = fs
      .readFileSync('./templates/ValidOracleAMM.mu.es')
      .toString()
  }
  const optionSellScriptTemplate = fs
    .readFileSync('./templates/Option_Sell.mu.es')
    .toString()
  const optionSellContract = Mustache.render(optionSellScriptTemplate, token)

  const symbolsTemplate = fs
    .readFileSync('./templates/symbols.json.mu')
    .toString()
  ...
  const optionSellContractFilename =
    buildDir + '/Option_Sell_' + tokenTicker + '.es'
  ...
  const optionSellCompiledContract = compileContract(
    optionSellContractFilename,
    symbolFileName,
  )
  ...
  jsTokenList.push(
    new UnderlyingToken(
      ...
      token.oracleType,
      token.icon,
      optionSellCompiledContract.scriptAddress,
    ),
  )
  console.log(`\x1b[32mSuccessfuly built contract for ${token.name} ...\x1b[0m`);

  
}

// Compile exercise request contract, linked with the option contract
const exerciseCompiledContract = compileContract(
  './Exercise_Option_Request.es',
  symbolFileName,
)
const sellFixedCompiledContract = compileContract(
  './Sell_Token_Request.es',
  symbolFileName,
)
...
const scriptConstantTemplate = fs
  .readFileSync('./templates/script_constants.js.mu')
  .toString()
const optionSellContract = Mustache.render(
  scriptConstantTemplate,
  scriptConstants,
)
